...
```{r, include = FALSE}
suggested_dependent_pkgs <- c("dplyr", "random.cdisc.data")
...
lyt <- basic_table() %>%
  ...
  split_rows_by("PARAM", split_fun = drop_split_levels, label_pos = "topleft", split_label = obj_label(adlb_f$PARAM), page_by = TRUE) %>%
  split_rows_by("AVISIT", split_fun = drop_split_levels, label_pos = "topleft", split_label = obj_label(adlb_f$AVISIT)) %>%
...
file_location <- file.path(output_dir, "example.docx")
...
export_as_docx(flex_tbl, file = file_location)
flex_tbl
```
### Pagination
```{r}
...
export_as_docx(flx_res, file = file_location)
flx_res[[1]]
```
## Contributions
